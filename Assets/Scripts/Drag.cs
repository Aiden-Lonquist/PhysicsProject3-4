...
public class Drag : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		float dragForce = CalculateDrag(velocityMagnitude);
		

		//Normalizing and find the dragVector
		// Comment out the line below:
		Vector3 dragVector = dragForce * -velocityVector.normalized;
	...
	{
		float forceDrag = dragConstant * Mathf.Pow(speed, velocityExponent);
		return forceDrag;
	}
}